...
({
	...
	createUI: function ()
	{
		...
		try {
			...
			jsnpa_m(jsnMenu).getElements('>.has-child>ul').addClass(this.dropdown_class);
			jsnpa_m(jsnMenu).getElements('>.has-child>ul li.has-child').addClass(this.submenu_dropdown_class);
			jsnpa_m(jsnMenu).getElements('>.has-child>ul>li>ul').addClass(this.dropdown_class);
...
//		});
	...
	getPageTitle: function (urlparams) {		
		...
		if (!pagetitle){
			if (jsnpa_mm('body').getElement('.pagetitle') == null) {
				return '';
			}
			pagetitle = jsnpa_mm('body').getElement('.pagetitle').textContent;
